...
///         Supports parseable permit-like selectors: EIP-2612 and DAI-style permit.
    ...
    IPolicyIntrospection
{
    ...
    function evaluate(
        ...
        (bool matched, address spender, bool isNonZeroApproval) = _parseApproval(data);
        ...
        if (knownSpenders[vault][token][spender]) {
            return (Decision.Allow, 0);
        }
    ...
    function onExecuted(
        ...
        knownSpenders[vault][token][spender] = true;
    }

    function _parseApproval(bytes calldata data)
        internal
        pure
        returns (bool matched, address spender, bool isNonZeroApproval)
    {
        if (data.length < 4) return (false, address(0), false);
        ...
        if (sel == APPROVE_SELECTOR) {
            if (data.length < 68) return (false, address(0), false);
            uint256 amount;
            assembly {
        ...
        if (sel == PERMIT_EIP2612_SELECTOR) {
            ...
            return (true, spender, value != 0);
        }

        // permit(address holder, address spender, uint256 nonce, uint256 expiry, bool allowed, ...)
        if (sel == PERMIT_DAI_SELECTOR) {
            if (data.length < 260) return (false, address(0), false);
            uint256 allowed;
            assembly {
                spender := and(calldataload(add(data.offset, 36)), 0xffffffffffffffffffffffffffffffffffffffff)
                allowed := calldataload(add(data.offset, 132))
            }
            return (true, spender, allowed != 0);
        }
